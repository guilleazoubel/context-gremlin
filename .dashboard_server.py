...
'''
    ...
    def refresh_pr(self, session_path, mode, force=False):
        ...
        try:
            ...
            git_opts = dict(cwd=str(repo_path), capture_output=True, text=True, stdin=subprocess.DEVNULL)

            old_commit = subprocess.run(
                ['git', 'rev-parse', 'HEAD'], **git_opts
            ).stdout.strip()

            # Clean up stale temp branch from any previous failed refresh
            subprocess.run(['git', 'branch', '-D', f'pr-{pr_number}-new'], **git_opts)
            ...
            try:
                ...
                self.send_error(504, 'Git fetch timed out after 60s — check network or auth')
                return
            ...
            new_commit = subprocess.run(
                ['git', 'rev-parse', f'pr-{pr_number}-new'], **git_opts
            ).stdout.strip()
            ...
            if old_commit == new_commit and not force:
                ...
                self.wfile.write(json.dumps({
                    'status': 'up_to_date',
                    'message': 'No new commits found. You can still force a re-review.'
                }).encode())
                return
            ...
            if old_commit == new_commit:
                ...
                log_result = subprocess.run(
                    ['git', 'log', '--oneline', f'{old_commit}..{new_commit}'], **git_opts
                )
                ...
                subprocess.run(['git', 'checkout', f'pr-{pr_number}-new'], **git_opts)
                subprocess.run(['git', 'branch', '-D', f'pr-{pr_number}'], **git_opts)
                subprocess.run(['git', 'branch', '-m', f'pr-{pr_number}'], **git_opts)

                # Get diff stats (changes since last review)
                changes_since = subprocess.run(
                    ['git', 'diff', '--stat', f'{old_commit}...pr-{pr_number}'], **git_opts
                ).stdout.strip()
...
- Fixed: Add ✅ prefix, e.g., `#### ✅ 1. normalizeString returns untrimmed value`
    ...
    function buildFindingsTracker(rawMarkdown, findings) {
        ...
        function getStatus(f, rawMarkdown) {
            ...
            for (let i = 0; i < patterns.length; i++) {
                if (rawMarkdown.indexOf(patterns[i]) > -1) return { label: 'Fixed', cls: 'status-fixed' };
            }
            // Also check if the heading line contains ✅ anywhere
            const re = new RegExp('####[^\\\\n]*' + num.replace(/[.*+?^${}()|[\\\\]\\\\]/g, '\\\\$&') + '[^\\\\n]*✅', 'i');
            if (re.test(rawMarkdown)) return { label: 'Fixed', cls: 'status-fixed' };
